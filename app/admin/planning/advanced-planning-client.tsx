        ...
        <div className="flex-1 p-2 overflow-hidden min-h-0" id="calendar-container">
          <FullCalendar
            ...
            eventContent={(arg) => {
              ...
              return (
                <div className={`flex flex-col text-[10px] leading-tight p-1 overflow-hidden w-full h-full ${isCancelled ? 'opacity-50' : ''}`}>
                  ...
                  <span className={`truncate text-[9px] font-bold opacity-80 ${isCancelled ? 'line-through' : ''}`}>{subtitle}</span>
                  {hasSubstitute && !isCancelled && (
                    <span className="truncate font-black text-amber-900 text-[8px] uppercase tracking-tighter">Par: {extendedProps.substitute}</span>
                  )}
                  {extendedProps.room && !isCancelled && <span className="truncate text-[8px] font-black uppercase tracking-widest text-slate-400 mt-auto">{extendedProps.room}</span>}
                </div>
              );
            }}
          />
