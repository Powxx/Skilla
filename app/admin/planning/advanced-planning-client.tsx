        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h2 className="text-lg font-semibold text-slate-800">Détails du cours</h2>
              <button onClick={() => setSelectedEvent(null)} className="text-slate-400 hover:text-slate-600">&times;</button>
            </div>
            
            <div className="p-6">
              <div className="mb-6 pb-4 border-b border-slate-100">
                <div className="font-semibold text-lg">{selectedEvent.extendedProps.subject}</div>
                <div className="text-slate-600 mt-1">Professeur : {selectedEvent.extendedProps.teacher}</div>
                <div className="text-slate-600">Classe : {selectedEvent.extendedProps.class}</div>
                <div className="text-slate-600">Date : {format(selectedEvent.start, 'dd/MM/yyyy HH:mm')} - {format(selectedEvent.end, 'HH:mm')}</div>
              </div>
              ...
              <form onSubmit={handleUpdateEventDetails} className="space-y-4">
                <label className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50 transition">
                  ...
                  />
                  <div>
                    <div className="text-sm font-medium text-slate-900">Marquer comme annulé / non réalisé</div>
                    <div className="text-xs text-slate-500">Le cours apparaîtra en rouge rayé pour les élèves.</div>
                  </div>
                </label>

                {!eventFormData.isCancelled && (
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Remplaçant (Optionnel)</label>
                    <select 
                    ...
                    <p className="text-xs text-slate-500 mt-1">Sélectionnez un autre professeur si l'original est absent.</p>
                  </div>
                )}

                <div className="pt-6 flex flex-col-reverse sm:flex-row justify-between gap-3">
                  <button type="button" onClick={handleDeleteSelectedEvent} className="px-4 py-2 text-red-600 hover:bg-red-50 rounded-lg font-medium border border-red-200 transition">
                    Supprimer le cours
                  </button>
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setSelectedEvent(null)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg font-medium transition">Fermer</button>
                    <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition disabled:opacity-50">
                      Mettre à jour
                    </button>
                  </div>
                </div>
