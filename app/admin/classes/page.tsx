...
import { getClassStudentsData } from "@/lib/classes-data";
...
export default async function AdminClassesPage(props: PageProps) {
  ...
  const classData = classId ? await getClassStudentsData(classId) : null;
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest ml-11">
              Suivi et regroupement des données élèves par classe
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm min-w-[240px]">
              <ClassSelector classes={classes} selectedClassId={classId} baseUrl="/admin/classes" />
            </div>
          </div>
        </header>
        ...
        {classData ? (
          <div id="class-report-content" className="space-y-8 p-4 bg-white/40 rounded-3xl border border-slate-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4 hover:scale-[1.02] transition-transform duration-300">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">À risque</p>
                  <p className="text-xl font-black text-rose-600 mt-1">{classData.stats.atRiskCount}</p>
                  <p className="text-[8px] font-bold text-rose-500/80 uppercase mt-0.5">élèves sous surveillance</p>
                </div>
              </div>
            </div>
                          ...
                          <tr key={student.id} className="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td className="px-6 py-4">
                              <div className="space-y-1">
                                {student.email ? (
                                  <a href={`mailto:${student.email}`} className="flex items-center gap-1.5 text-slate-600 hover:text-blue-600 transition-colors">
                                    <Mail className="h-3 w-3 text-slate-400" />
                                    <span className="truncate max-w-[150px] inline-block">{student.email}</span>
                                  </a>
                                ) : null}
                                {student.phone ? (
                            ...
                            <td className="px-6 py-4 text-right">
                              <Link
                                href={`/admin/students/${student.id}/dispenses`}
                                className="inline-flex items-center justify-center p-1.5 hover:bg-slate-100 rounded-xl text-slate-400 hover:text-blue-600 transition"
                              >
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-12 text-center shadow-sm max-w-xl mx-auto mt-8 flex flex-col items-center">
            ...
            <p className="mt-2 text-xs text-slate-500 leading-relaxed max-w-sm">
              Sélectionnez une classe dans le menu déroulant ci-dessus pour afficher la liste des élèves correspondants, leurs moyennes, points de conduite et assiduité.
            </p>
          </div>
        )}
      </div>
