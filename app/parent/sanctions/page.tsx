...
export default async function ParentSanctionsPage({
  searchParams,
}: {
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <header className="mb-8">
        ...
        <p className="mt-2 text-sm text-slate-500 font-medium">
          Dossier disciplinaire de votre enfant : <span className="text-slate-800 font-bold">{student.firstName} {student.lastName}</span> ({student.class?.name || "Sans classe"})
        </p>
      </header>
      ...
      {sanctions.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-12 text-center">
          ...
          <p className="text-xs text-slate-500 mt-1 font-medium">
            Votre enfant ne fait l'objet d'aucune sanction ou avertissement en cours.
          </p>
        </div>
      ) : (
