...
export default function SkinDefense() {
  ...
  // Game Loop: Enemies (Top to Bottom in col 1)
  useEffect(() => {
    if (gameOver) return;
    const interval = setInterval(() => {
        setEnemies(prev => [...prev, { id: Date.now(), r: 0, c: 1, health: 100 * wave }]);
    }, 2000);
  ...
  useEffect(() => {
    ...
    const loop = setInterval(() => {
        ...
        setEnemies(prev => prev.map(e => ({...e, r: e.r + 0.2}))
            .filter(e => {
                if (e.r >= GRID_SIZE) {
                    setLives(l => {
        ...
        setEnemies(prev => {
            ...
            towers.forEach(t => {
                // Tower at (r, c) targets enemies nearby
                const target = nextEnemies.find(e => Math.abs(e.r - t.r) <= 1 && Math.abs(e.c - t.c) <= 1);
                if (target) {
                    target.health -= (powerUps.damageBoost ? 25 : 10);
                }
            });
  ...
  const placeTower = (r: number, c: number) => {
    if (score < TOWER_COST || towers.find(t => t.r === r && t.c === c) || c === 1) return; // Cant place on path
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-white p-4 font-sans">
      ...
      <div className="bg-slate-800 p-2 rounded-3xl border-4 border-slate-700 shadow-2xl">
        <div className="grid" style={{ gridTemplateColumns: `repeat(3, 40px)` }}>
            {Array.from({length: 10*3}).map((_, i) => {
                const r = Math.floor(i / 3);
                const c = i % 3;
                const isTower = towers.find(t => t.r === r && t.c === c);
                const enemy = enemies.find(e => Math.floor(e.r) === r && Math.floor(e.c) === c);
                const isPath = c === 1;
                return (
                    <div key={i} onClick={() => placeTower(r, c)} className={`w-[40px] h-[40px] border border-slate-700/50 flex items-center justify-center cursor-pointer ${isPath ? 'bg-slate-900' : 'hover:bg-slate-700'}`}>
                        {isTower ? '🧴' : enemy ? '🦠' : ''}
                    </div>
                );
            })}
