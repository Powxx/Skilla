...
export default async function EmployerSanctionsPage({
  searchParams,
}: {
  ...
  const [student, sanctions] = await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <nav className="mb-6 text-xs font-bold uppercase tracking-widest text-slate-400">
        <Link href={`/employer/dashboard${suffix}`} className="hover:text-slate-600 transition-colors">
          Synthèse
        </Link>
      ...
      <header className="mb-8">
        ...
        <p className="mt-2 text-sm text-slate-500 font-medium">
          Dossier disciplinaire de votre alternant : <span className="text-slate-800 font-bold">{student.firstName} {student.lastName}</span> ({student.class?.name || "Sans classe"})
        </p>
      </header>
      ...
      {sanctions.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-12 text-center">
          ...
          <p className="text-xs text-slate-500 mt-1 font-medium">
            L'alternant ne fait l'objet d'aucune sanction ou avertissement en cours au CFA.
          </p>
        </div>
      ) : (
